// Adapted from desktop-cc-gui effort-flame.tsx; keep the host flame shader and starfield.
import { React, useEffect, useRef, useState } from "../react-context";
...
function Starfield() {
  ...
  return <canvas ref={canvasRef} aria-hidden className="ms-effort-canvas" />;
}
...
function PixelationOverlay() {
  ...
  return (
    <div
      aria-hidden
      className="ms-effort-flame"
    >
      {/* accent-300 wash bleeding in from the right, under the pixel noise */}
      <div className="ms-effort-pixel-wash" />
      <canvas ref={canvasRef} className="ms-effort-canvas" />
    </div>
  );
}
...
function createFlameProgram(gl: WebGLRenderingContext): WebGLProgram | null {
  ...
  const vertex = compile(gl.VERTEX_SHADER, FLAME_VERT);
  const fragment = compile(gl.FRAGMENT_SHADER, FLAME_FRAG);
...
export function FlameOverlay() {
  ...
  return (
    ...
    >
      ...
      <div className="ms-effort-flame-wash" />
      <Starfield />
      <canvas ref={canvasRef} className="ms-effort-canvas" />
    </div>
  );
