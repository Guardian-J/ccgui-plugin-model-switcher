...
> **供应商切换：** 旧宿主保留会话渠道回调；没有该回调的新宿主通过原生配置 IPC 切换，先显示宿主返回的文件路径并确认全局影响，成功后才应用模型和保存选择。取消或 IPC 失败不会继续切模型；等待期间换会话会停止后续操作。创建、编辑和删除独立渠道同样在可能改写原生文件前确认。独立渠道支持 Claude / Codex / Kimi / Grok / OMP / PI。DSH、AGY、OpenCode、Qoder 等使用自身账号或服务配置，暂不支持独立渠道。系统渠道详情只读。OMP / PI 的独立渠道还会写入 `models.yml` / `models.json`，并需指定 `api` 协议。
...
| 切换渠道 | 有 `onChannelChange` 的宿主保留会话绑定；无该回调的新宿主确认后调用 `set_current_provider`，改变全局 CLI 原生配置，不伪造会话级绑定；OMP / PI 继续使用完整的供应商/模型 ID |
| 删除独立渠道 | 删除插件记录，并从宿主供应商列表移除；OMP / PI 同时从模型配置移除该供应商；若删的是当前项，切回可用的系统渠道或原生配置 |

CLI 配置如何应用取决于宿主能力：会话回调路径等待宿主已提交的渠道值；原生配置路径先查询 `provider_file_paths`，非空时须确认，查询失败则停止，不猜测覆盖路径。原生配置使用宿主保留 ID `__local_settings_json__`，不会误选名为 `local` 的自定义渠道。切换会话会中止后续模型应用和保存，但已经提交给宿主的文件写入不会自动撤销。OMP / PI 独立渠道仍会改写模型配置文件。暂不支持独立渠道的 CLI 可选择原生配置，并删除旧版保存的独立渠道。用户主动执行的提示词清洗另见下文。
...
| 问题 | 检查方式 |
| --- | --- |
| 切换渠道后 CLI 配置被改写 | 新宿主的预期行为：确认面板会先列出受影响文件并说明全局影响；取消不会写入。旧宿主沿用其会话回调行为；OMP / PI 独立渠道仍管理原生模型配置 |
...
`/scripts/check-channel-switch.html` 验证旧宿主延迟确认、重复点击、失败重试和保存顺序；加 `?host=native` 验证原生配置确认/取消、写入前保护、IPC 拒绝、原生与 `local` ID 区分及同 CLI 换会话竞态；加 `?engine=dsh`（或 `agy`、`opencode`、`qoder`、`qoder-cn`）验证不支持的渠道被禁用、原生入口可选及最后一个旧渠道可删除。加 `&manual=1` 可保留页面手动操作。
